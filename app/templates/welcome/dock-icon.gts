  ...
  <div class="flex flex-col h-full w-full">
    ...
    <div class="flex-auto grow mb-2 mt-2 text-main-text">
      ...
      <div class="flex h-full justify-center p-2 w-full">
        <ToggleSwitch data-test-show-dock-icon-toggle @checked={{@controller.settings.showDockIcon}} @onClick={{set @controller "settings.showDockIcon" (not @controller.settings.showDockIcon)}} />
      </div>
    </div>

    <div class="flex justify-between mt-8 w-full">
      <LinkTo class="btn btn-secondary flex-1 mr-1 p-2 text-center text-sm" @route="welcome.auto-start">
        Previous
      </LinkTo>

      <LinkTo class="btn btn-primary flex-1 ml-1 p-2 text-center text-sm" @route="welcome.cloud-sync">
        Next
      </LinkTo>
    </div>
  </div>
