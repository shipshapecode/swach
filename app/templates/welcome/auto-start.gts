  ...
  <div class="flex flex-col h-full w-full">
    ...
    <div class="flex-auto mb-2 mt-2 text-main-text">
      ...
      <div class="flex h-full justify-center p-2 w-full">
        <ToggleSwitch data-test-auto-start-toggle @checked={{@controller.settings.openOnStartup}} @onClick={{set @controller "settings.openOnStartup" (not @controller.settings.openOnStartup)}} />
      </div>
    </div>

    <div class="flex justify-between mt-8 w-full">
      <LinkTo class="btn btn-secondary flex-1 mr-1 p-2 text-center text-sm" @route="welcome.index">
        Previous
      </LinkTo>

      <LinkTo data-test-link-dock-icon class="btn btn-primary flex-1 ml-1 p-2 text-center text-sm" @route="welcome.dock-icon">
        Next
      </LinkTo>
    </div>
  </div>
