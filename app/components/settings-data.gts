...
export default class SettingsData extends Component {<template><div class="p-4 h-full">
  ...
  <OptionsMenu data-test-settings-format-dropdown class="my-4 w-full" @optionsClasses="divide-y divide-alt !p-0 w-full" @triggerClasses="bg-menu font-medium inline-flex justify-between px-4 py-2 rounded-md text-sm text-menu-text w-full focus:outline-none">
    <:trigger>
    ...
    <:content>
      {{#each this.colorFormats as |format|}}
        <button data-test-format-option={{format}} class="px-4 py-2 text-left text-sm transition-colors w-full hover:text-menu-text-hover" type="button" {{on "click" (set this "settings.defaultColorFormat" format)}}>
          {{format}}
        </button>
      {{/each}}
    </:content>
  ...
  <div class="mb-16">
    ...
    <LoadingButton data-test-export-swatches-button class="mb-2" @loading={{this.isExporting}} @onClick={{this.exportIndexedDB}}>
      Backup swatches
    </LoadingButton>

    <LoadingButton data-test-import-swatches-button @loading={{this.isImporting}} @onClick={{this.importIndexedDB}}>
      Restore from backup
    </LoadingButton>
  </div>
</div></template>
